# ADR 0011: 경로 가능성 연구와 관측·카메라 통합 Gate 분리
...
> 추상 `ActorTrack` 또는 후속 카메라 인식 결과로 R2-A 기동을 판단할 수 있는가?
...
- 공개 audit 실행 완료
- Ideal Actor 출현 coverage hard failure 2건
- fresh EMPTY online false-safe 계약 미해결
- 카메라 모델 미구현
...
- R5를 먼저 수행하더라도 `Ideal ActorTrack path/controller lane`으로 표시하고 카메라 통합
  결과로 부르지 않는다.
- R2-B를 통과하기 전 perception-integrated R6와 hidden을 실행하지 않는다.
- 실제 카메라·센서·사람 탑승 안전성은 후속 팀 합의와 별도 증거가 필요하다.
