...
- 실제 초음파 거리·배치 coverage·반사·무응답 계약: 미구현
- formal R5-C/R6·hidden·제품 자격: 미완료

이번 결과는 사람이 위험 관련 구역에 들어오기 전부터 같은 track으로 감시되는 추상 공개
시뮬레이션 조건만 검증한다. 실제 초음파 센서가 그 접근 구간을 안정적으로 측정하거나 2차원
위치·속도 track을 제공한다는 뜻이 아니다.

> 후속 기준: [초음파 관측 전환 명세](22-ultrasonic-observation-transition.md). 원본 실패와
> 이 파생 결과는 모두 유지하며, 거리 관측 하네스가 생기기 전에는 실제 센서 R2-B 통과로
> 승격하지 않는다.
...
있다”는 추상 입력이며 실제 초음파 센서 설치 위치·방향·유효 거리를 확정하지 않는다.
...
1. 실제 초음파 연구를 시작할 때 센서 배치·빔 경계·반사·무응답·거리 누락을 별도 계약으로 만든다.
