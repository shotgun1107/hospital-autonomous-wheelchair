...
6. `R4-6`: clean source 전체 public qualification·receipt·마지막 전체 회귀

현재 구현·검증된 범위는 `R4-1~R4-6`이다. R4-2는 feasible·독립 검증·hash/provenance가 일치한
...
통과했다. R4-4는 pose·heading·이전 cursor를 함께 사용해 self-overlap을 구분하며 whole-section
...
partial/complete/receipt 수명주기를 구현했다. clean commit `f43fbbf`의 R4-6에서 public
`21/21`, hard·relation failure 0, serial/process parity와 repeat determinism PASS, receipt 생성을
확인했고 전체 회귀 `794 passed`를 완료했다. 세부 증거는
[`R4 공개 qualification 결과`](r4-public-local-reference-qualification-result-2026-08-14.md)에
기록했다. reference가
...
- anchor connector는 source anchor 의미를 유지한다. 기하 변위로 전진·후진을 추론하지 않고
  `travel_direction=NONE`인 비실행 연결 구간으로 보존하며, 변위가 있으면 양 끝 정지 표식을
  요구한다.
...
- 한 translation section에는 한 방향만 허용한다. forward/reverse가 바뀌면 section을 분리하며
  `ROTATE`, `HOLD`와 모든 abstract anchor connector는 `travel_direction=NONE`이다.
...
- translation primitive는 `FORWARD` 또는 `REVERSE`, 회전·HOLD와 R3의 추상
  `ANCHOR_CONNECTOR`는 `NONE`으로 기록한다.
- `ANCHOR_CONNECTOR`의 기하 변위에서 전진·후진을 추측하지 않는다. 변위가 있는 connector는
  양 끝 `STOP_MARKER`와 stopped entry/exit가 있는 비실행 연결 구간으로만 허용한다.
