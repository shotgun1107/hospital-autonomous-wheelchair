...
- 비범위: 실제 카메라·FOV·가림·검출 성능, hidden, 제품 알고리즘 채택
...
현재 하네스에는 카메라 가시영역이나 진입구 감시 증거가 없다. 따라서 fresh `EMPTY`는
...
3. 실제 카메라·가시영역·가림·검출 실패를 포함한 보수적 관측 계약

현재는 어느 것도 구현하지 않으므로 R2-B hard failure 2건은 종료하지 않는다.
