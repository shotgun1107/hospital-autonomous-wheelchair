...
| `DYN-R4-001` | 검증된 R2/R3 source를 WAIT/LEFT/RIGHT immutable full reference와 revision-bound sliding subpath로 변환하고, 동일 path의 window 이동은 controller session을 유지하며 stale revision·과거 session 결과는 거부한다. | [R4 상세 명세](../../docs/research/dynamic-actor-experiment/15-local-maneuver-reference-contract.md), [ADR 0012](../../docs/decisions/0012-persistent-controller-session-for-sliding-subpaths.md), [contracts](src/hospital_path_lab/local_reference_contracts.py), [builder](src/hospital_path_lab/local_reference_builder.py), `tests/test_local_reference_contracts.py`, `tests/test_local_reference_builder.py` | R4-1 계약·semantic hash·revision/session/lifecycle와 R4-2 R3 source→canonical LEFT/RIGHT builder L1 구현. 직접 builder 9개, R4/R3 영향권 59개 통과. 대표 public `wide-straight-left`를 23 knot·7 section·2 rotation section으로 변환하고 minimum clearance `0.244508m`를 보존했다. R2 temporal evidence·독립 R4 validator·window manager·public runner·R5 연결은 미구현이며, R4 reference는 이동 허가·동적 안전·controller 추종·제품 알고리즘 증거가 아니다. |
