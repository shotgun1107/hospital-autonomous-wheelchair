...
| `DYN-R5-001` | 같은 R4 full reference를 persistent RPP·source-derived DWB가 공통 section executor와 shared gate 아래 추종한다. local window endpoint와 full terminal을 분리하고, same-session subgoal update에서는 controller·stateful critic을 유지하며 모든 result에 reference identity를 왕복 결박한다. | [R5 상세 명세](../../docs/research/dynamic-actor-experiment/16-persistent-controller-comparison.md), [ADR 0013](../../docs/decisions/0013-common-reference-section-executor.md), [ADR 0012](../../docs/decisions/0012-persistent-controller-session-for-sliding-subpaths.md), [R5-1 contracts](src/hospital_path_lab/persistent_controller_contracts.py), [R5-1 tests](tests/test_persistent_controller_contracts.py) | R5-1 계약·binding L1 연결. R5-A `SPATIAL_ONLY` full reference·exact window·grid·vehicle·stop epoch·delivery tick을 결박하고 same-tick idempotence, subgoal state 유지, 새 session reset, planned/protective stop 분리를 검증했다. 전용 `12 passed`, 당시 R4 직접 영향권 `68 passed`. RPP/DWB adapter, shared gate, public receipt와 R5-B/C는 미구현이며 controller 추종·Actor online 안전·제품 채택 증거가 아니다. |
| `DYN-R5-002` | RPP와 DWB의 translation 계산 밖에서 planned stop·실제 정지 확인·원자적 제자리회전·terminal dwell·HOLD를 같은 persistent executor로 실행한다. gate 보호정지 중에는 section/counter를 진행하지 않고, HOLD는 자동 해제하지 않는다. | [R5 상세 명세](../../docs/research/dynamic-actor-experiment/16-persistent-controller-comparison.md), [ADR 0013](../../docs/decisions/0013-common-reference-section-executor.md), [executor](src/hospital_path_lab/reference_section_executor.py), [tests](tests/test_reference_section_executor.py) | R5-2 L1 연결. 제한 감속, 선·각 실제 정지 3 tick, shortest rotation, terminal 별도 10 tick dwell, 같은 tick cache, 새 session reset, causal R4 window 재생을 검증했다. 전용 `13 passed`, R5-1/R4 직접 영향권 `81 passed`. translation pose는 합성 feedback이며 RPP/DWB closed loop·shared gate·public receipt·제품 채택 증거가 아니다. |
| `DYN-R5-003` | persistent RPP는 current window의 active translation section만 lookahead에 사용하고, full-reference active section progress와 명시적 stop/terminal remaining으로만 감속한다. common executor 결과를 reference-bound result로 만들고 post-apply 2초·41-pose rollout을 생성한다. | [R5 상세 명세](../../docs/research/dynamic-actor-experiment/16-persistent-controller-comparison.md), [RPP adapter](src/hospital_path_lab/persistent_rpp_controller.py), [RPP tests](tests/test_persistent_rpp_controller.py), [window manager](src/hospital_path_lab/local_reference_window.py) | R5-3 L1 연결. public `wide-straight-left` 20Hz static closed loop가 `20.75s`, reset `1`, window update `4`, max tracking error 약 `0.09923m`로 terminal 완료했다. 동일 위치 회전에서는 prior monotonic cursor를 heading보다 먼저 적용하는 회귀도 닫았다. 전용 `8 passed`, R5-1·R5-2·window 직접 영향권 `50 passed`, R4 확장 영향권 `124 passed`. DWB·shared gate·8-case public receipt·Actor 안전·제품 채택 증거는 아니다. |
