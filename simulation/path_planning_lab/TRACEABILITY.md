...
| `DYN-R4-001` | 검증된 R2/R3 source를 WAIT/LEFT/RIGHT immutable full reference와 revision-bound sliding subpath로 변환하고, 동일 path의 window 이동은 controller session을 유지하며 stale revision·과거 session 결과는 거부한다. | [R4 상세 명세](../../docs/research/dynamic-actor-experiment/15-local-maneuver-reference-contract.md), [ADR 0012](../../docs/decisions/0012-persistent-controller-session-for-sliding-subpaths.md), [contracts](src/hospital_path_lab/local_reference_contracts.py), [builder](src/hospital_path_lab/local_reference_builder.py), [validator](src/hospital_path_lab/local_reference_validation.py), [window manager](src/hospital_path_lab/local_reference_window.py), `tests/test_local_reference_contracts.py`, `tests/test_local_reference_builder.py`, `tests/test_local_reference_validation.py`, `tests/test_local_reference_window.py` | R4-1 계약·semantic hash·revision/session/lifecycle, R4-2 builder, R4-3 독립 validator와 R4-4 pose-heading/cursor 기반 whole-section sliding window manager를 L1 구현. window 전용 16개, 구현 직후 R4/R3 영향권 77개 통과. 대표 public `wide-straight-left`는 23 knot 전 순회를 같은 session으로 처리해 subgoal revision `0→4`, terminal window `(15..22)`까지 통과했다. R2 temporal evidence·public runner·R5 연결은 미구현이며, R4 reference/window는 이동 허가·동적 안전·controller 추종·제품 알고리즘 증거가 아니다. |
