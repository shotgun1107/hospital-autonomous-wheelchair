...
| `DYN-CTRL-003` | 동적 DWA는 후진 없이 최대 217후보·후보당 41 pose, v5 절대 비용과 결정론 tie-break를 사용한다. 양의 선속도 후보가 없으면 제자리 회전 대신 `NO_SAFE_CANDIDATE`를 반환하고 terminal stopping을 공통 gate 계약으로 재검사한다. | [dwa.py](src/hospital_path_lab/local_algorithms/dwa.py), [dynamic_safety.py](src/hospital_path_lab/dynamic_safety.py), `tests/test_dynamic_dwa_pipeline.py::test_dynamic_dwa_uses_217_candidates_and_41_post_apply_poses`, `::test_dynamic_dwa_cost_equations_match_the_frozen_oracle`, `::test_dynamic_dwa_reverse_is_disabled_and_zero_does_not_add_a_sample`, `::test_dynamic_dwa_rejects_every_candidate_when_terminal_stop_is_blocked` | 연결됨, L1. 사용자 정의 연구용 DWA이며 Nav2 DWB 자체가 아니다. |
...
| ID | 연구 요구사항 | 구현·시험 근거 | 상태·증거 한계 |
|---|---|---|---|
| `DYN-EVAL-001` | controller가 보지 못한 실제 Actor 상태와 20 Hz chassis trace를 200 Hz로 재적분하고 sample 사이 상대이동 상한까지 차감해 collision·`0.08m` clearance·금지구역을 hard 판정한다. | [dynamic_evaluation.py](src/hospital_path_lab/dynamic_evaluation.py), [collision.py](src/hospital_path_lab/collision.py), `tests/test_dynamic_evaluation.py::test_swept_margin_detects_collision_between_5ms_samples`, `::test_forbidden_entry_is_a_hard_failure_not_a_metric_only_warning` | 연결됨, L1. 보수적 수치기하 평가이며 연속 실물 swept-volume 증명은 아니다. |
...
| ID | 연구 요구사항 | 구현·시험 근거 | 상태·증거 한계 |
|---|---|---|---|
| DYN-RUN-001 | 공개 corpus와 공통 fault 자격을 hidden 전에 판정하고 code·parameter·corpus hash가 있는 manifest를 먼저 동결한다. | [dynamic_runner.py](src/hospital_path_lab/dynamic_runner.py), `tests/test_dynamic_runner.py::test_runner_writes_hashed_manifest_results_markdown_and_png` | 연결됨, L2. 축소 runner 시험은 승격 근거가 아니다. |
...
| DYN-HID-001 | commitment와 실제 seed 일치, public/hidden hash 분리, hidden 클래스별 5개, 소비 영수증과 재사용 거부를 강제한다. | [dynamic_corpus.py](src/hospital_path_lab/dynamic_corpus.py), [dynamic_runner.py](src/hospital_path_lab/dynamic_runner.py), `tests/test_dynamic_hidden_lifecycle.py`, `tests/test_dynamic_runner.py::test_consumed_hidden_commitment_cannot_be_reused` | 연결됨, L1/L2. hidden은 절대 비밀 데이터가 아니라 절차적 과적합 방지 split이다. |
