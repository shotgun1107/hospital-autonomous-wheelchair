...
| `DYN-OBS-002` | source·episode·map·sequence·revision·hash·binding·profile σ·시간 순서를 transactional하게 검증하고 `age == 300ms`는 fresh, 초과는 stale로 둔다. prediction은 validator가 만든 fresh snapshot만 받는다. | [dynamic_observation.py](src/hospital_path_lab/dynamic_observation.py), [dynamic_prediction.py](src/hospital_path_lab/dynamic_prediction.py), `tests/test_dynamic_observation.py::test_each_source_identity_mismatch_returns_a_structured_reason`, `::test_sequence_revision_hash_duplicate_and_binding_faults_are_transactional`, `::test_rehashed_uncertainty_smaller_than_the_active_profile_is_rejected`, `::test_no_frame_event_must_advance_sequence_and_delivery_time`, `::test_ttl_exactly_300ms_is_fresh`, `::test_ttl_any_later_nanosecond_is_stale`, `tests/test_dynamic_prediction.py::test_raw_frame_and_not_yet_delivered_snapshot_are_rejected`, `::test_hash_invalid_snapshot_cannot_reach_prediction` | 제한적 연결, L1. invalid/stale 입력의 새 비영점 명령 차단은 3단계 gate 대상이며 아직 구현하지 않았다. |
| `DYN-SAFE-001` | 관측 age, 50 ms 적용 지연, vector `0.50m/s` clamp, 2σ와 임의방향 가속 reachable bound로 controller 비종속 time-indexed 원형 tube를 만든다. | [dynamic_prediction.py](src/hospital_path_lab/dynamic_prediction.py), `tests/test_dynamic_prediction.py::test_actor_tube_matches_independent_center_sigma_and_acceleration_oracle`, `::test_piecewise_acceleration_bound_is_continuous_at_velocity_delta_time`, `::test_radius_is_finite_and_monotonic_for_all_frozen_profiles`, `::test_speed_cap_preserves_vector_direction_instead_of_clamping_each_axis`, `::test_speed_cap_preserves_direction_for_extreme_finite_components` | 제한적 연결, L1. tube 수식만 검증했으며 `0.08m` actual clearance, swept geometry, safety gate와 실제 사람 안전성은 미구현·미증명이다. |
