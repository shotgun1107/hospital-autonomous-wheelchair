...
서버 Python 코드가 R7의 기존 controller·safety gate를 같은 프로세스에서 직접 호출할 때의
함수·입력·반환은
[R7 runtime 함수 계약](../../docs/integration/r7-runtime-function-contract.md)을 따른다.
설치와 개념 설명은
[R7 Python runtime 연결 안내](../../docs/integration/r7-runtime-interface.md)에 있다. 이는
HTTP 서버나 제품 알고리즘 채택 문서가 아니다.
...
### R7 단일 관측 누락 TTL 재사용과 원 경로 복귀 — 2026-08-18

방향 계산이 끝난 뒤 마지막 유효 frame이 `300ms` TTL 안에 있을 때만 기존 이동을 계속하도록
고쳤다. 재사용 frame은 방향 history나 안전 frame 수를 늘리지 않으며 최초 출발·재출발·새
추월 완료를 허가하지 않는다. 사람 통과 확인 뒤에는 계획 정지, 새 `stop_epoch`, 서로 다른
새 안전 frame 11개, 원 경로용 새 session 순서로 복귀한다.
...
외부 gate 장시간 재생에서 명시적 stop을 지나 2초 직진하는 비현실적 rollout 때문에 발생했던
tick `89`의 false static rejection도 재현했다. 안전 기준과 gate를 유지한 채, stop/terminal 앞
translation이 한 명령 구간 뒤 제한 감속·정지·hold하는 41-pose fallback을 제출하도록 고쳤다.
대표 경로는 tick `417`·simulation time `20.85s`, 최소 정적 여유 약 `0.15967m`, external gate
rejection `0`으로 terminal까지 완료했다.
...
보존한다. 사용자는 후속 Python 연구에서 section-bound 제한 후진을 허용했다. R4 v2는 source
